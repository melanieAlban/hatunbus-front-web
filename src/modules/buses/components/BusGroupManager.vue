    ...
    <div style="padding:1rem">
      ...
      <div v-else>
        <div v-for="b in groupBuses" :key="b.id" style="padding:0.5rem;border-bottom:1px solid var(--gray-light);display:flex;justify-content:space-between;align-items:center">
          <div style="display:flex;align-items:center;gap:0.75rem">
            <span 
              :style="{ 
                ...
                height: '10px', 
                borderRadius: '50%', 
                backgroundColor: b.status === 'ACTIVE' ? '#22c55e' : '#ef4444',
                flexShrink: 0
              }"
            ></span>
            <div>
          ...
          <div style="display:flex;gap:0.5rem;align-items:center" class="bus-actions">
            <Button class="p-button-text p-button-rounded" icon="pi pi-eye" aria-label="Ver" @click="() => openBusDetail(b)"></Button>
            <span 
              class="power-btn"
              :class="b.status === 'ACTIVE' ? 'power-active' : 'power-inactive'"
              role="button"
              tabindex="0"
              :aria-label="b.status === 'ACTIVE' ? 'Desactivar' : 'Activar'"
              @click="() => toggleBusStatus(b)"
              @keydown.enter="() => toggleBusStatus(b)"
            >
...
async function toggleBusStatus(bus: any) {
  try {
    ...
    if (gbIdx !== -1) {
      // Crear una copia del objeto con el nuevo estado para forzar reactividad
      groupBuses.value[gbIdx] = { ...groupBuses.value[gbIdx], status: target }
      // Forzar actualización del array
      groupBuses.value = [...groupBuses.value]
    }
    
    // if detail open, update selectedBus
    if (selectedBus.value && selectedBus.value.id === bus.id) {
      selectedBus.value = { ...selectedBus.value, status: target }
    }
    
    // Emitir evento para que la vista padre actualice la tabla general de buses
    emit('refresh')
    
    success('Estado actualizado', `Bus ${bus.plate} ahora está ${target === 'ACTIVE' ? 'activo' : 'inactivo'}`)
  } catch (err: any) {
...
/* Estilos para los botones de acciones de buses */
.bus-actions :deep(.p-button-text) {
  color: var(--app-text) !important;
}

.bus-actions :deep(.p-button-text:hover) {
  background-color: var(--surface-100) !important;
}

.bus-actions :deep(.p-button-text:focus),
.bus-actions :deep(.p-button-text:active) {
  background-color: var(--surface-200) !important;
  box-shadow: none !important;
}

/* Botón de power personalizado */
.power-btn {
...
.power-btn.power-active .pi {
  color: #22c55e !important;
}

.power-btn.power-inactive .pi {
